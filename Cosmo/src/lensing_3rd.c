			       ...
			       error **err)
{
   ...
   res->lens = init_parameters_lens(OMEGAM, OMEGADE, W0_DE, W1_DE, W_POLY_DE, N_POLY_DE,
				    ...
				    NEFFNUMASS, NORM, NSPEC, Nzbin, Nnz, nofz, photz, par_nz,
				    NONLINEAR, TRANSFER, GROWTH, DEPARAM,
				    NORMMODE, tomo_all, limber, reduced_none, 0.0,
...
cosmo_3rd *copy_parameters_3rd_only(cosmo_3rd *source, error **err)
{
   ...
   res = init_parameters_3rd(source->lens->cosmo->Omega_m,source->lens->cosmo->Omega_de,
			     ...
			     source->lens->redshift->nofz, source->lens->redshift->photz, source->lens->redshift->par_nz,
...
double n_eff_one(cosmo *self, double k, error **err)
{
   ...
   testErrorRetVA(!isfinite(n), ce_infnan, "inf or nan encountered, k = %g",
...
double Q3(double n, error **err)
{
   ...
   testErrorRetVA(!isfinite(q3), ce_infnan, "inf or nan encountered, (n, erg) = (%g, %g)",
...
double ascocou(cosmo_3rd *self, double a, double k, error **err)
{
   ...
   testErrorRetVA(!isfinite(erg), ce_infnan, "inf or nan encountered, (s8, q3, q4, erg) = (%g, %g, %g, %g)",
...
double bscocou(cosmo_3rd *self, double a, double k, error **err)
{
   ...
   testErrorRetVA(!isfinite(erg), ce_infnan, "inf or nan encountered, (n, q, erg) = (%g, %g, %g)",
...
double cscocou(cosmo_3rd *self, double a, double k, error **err)
{
   ...
   testErrorRetVA(!isfinite(erg), ce_infnan, "inf or nan encountered, (n, twoq, erg) = (%g, %g, %g)",
...
double int_for_B_kappa_bar1(double a, void *intpar, error **err)
{
   ...
   testErrorRet(!isfinite(erg), ce_infnan, "inf or nan encountered", *err, __LINE__, 0.0);
...
double B_kappa_bar(cosmo_3rd *self, double s1, double s2, int abc, int i_bin, int j_bin, int k_bin, error **err)
{
                  ...
                  for (j=0,s2log=logsmin; j<N_s2; j++,s2log+=ds) {
                     ...
                     if (self->bispmode==PT) {
                        ...
                        for (m=0; m<=2; m++) {
                           ...
                           f2 = 0.0;
#endif
                           //if (!isfinite(f1 + f2) || f1+f2 < 0) { printf("f1+f2 = %g for (s1,s2) = (%g,%g), #(%d,%d)\n", f1+f2, ss1, ss2, i, j); }
   ...
   testErrorRet(!isfinite(f1), ce_infnan, "inf or nan", *err, __LINE__, 0.0);
...
double int_for_map3_3d(double x[], size_t dim, void *intpar)
{
   ...
   testErrorRet(!isfinite(res), ce_infnan, "inf or nan", *err, __LINE__, 0.0);
   ...
   testErrorRetVA(!isfinite(res), ce_infnan, "inf or nan for (l1, l2)=(%g,%g)", *err, __LINE__, 0.0, l1, l2);
 ...
 * Do not use this function directly, call map3 instead!         *
 * ============================================================= */
double map3_perm(cosmo_3rd *self, double R[3], int i_bin, int j_bin, int k_bin, filter_t wfilter, error **err)
{
   ...
   testErrorRet(!isfinite(res), ce_infnan, "inf or nan", *err, __LINE__, -1.0);
...
double map3_SLC_t1(cosmo_3rd *self, double R, int n_bin, error **err)
{
   ...
   for (a1=amin+da; a1<0.999; a1+=da) {
      ...
      for (a2=a1+da; a2<0.999; a2+=da) {
         ...
         testErrorRet(!isfinite(res), math_infnan, "res not finite", *err, __LINE__, 0.0);
