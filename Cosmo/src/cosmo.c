/* ============================================================ *
 * cosmo.c							                                  *
 * Martin Kilbinger, Karim Benabed 2006 -               			 *
 *								*
		       ...
		       error **err)
{
   ...
   //printf("N_a, da, a_min = %d %g %g\n", res->N_a, (1.0 - res->a_min) / (res->N_a - 1), res->a_min); //, N_a_min);
...
void consistency_parameters(const cosmo *self, error **err)
{
   ...
   /* Without this test here, there are NaNs in Tsqr */
...
void updateFrom(cosmo* avant, cosmo* apres, error **err)
{
   ...
   if (change_P_NL(avant,apres)) {
      ...
      /* A bit of overkill; The coyote13          *
...
// NEW function: P_L instead of P_L_nonorm
double int_for_sigma_R_norm(double logk, void *intpar, error **err)
{
   double res, k, x, R, tt;
...
// NEW function
double sigma_8_sqr_norm(cosmo* self, error **err)
{
   double integral, cmp_sigma8;
   cosmoANDdouble cANDd;

   cANDd.self = self;
   cANDd.r    = 8.0;    /* [Mpc/h] */
   integral = sm2_qromberg(int_for_sigma_R_norm, (void*)&cANDd, log(k_min), log(k_max), 1.0e-6, err);
   forwardError(*err,__LINE__,0);
   cmp_sigma8 = 4.5/pi_sqr*integral;
...
/* Prefactor stemming from relation between curvature, potential, and density perturbations *
 * using the Poisson equation.                                                              */
double pref_pert_poiss(double Omega_m, double n_spec, double h_100, double k_pivot)
{
   double pref;

   pref  = 4.0/25.0 / dsqr(Omega_m) * dsqr(dsqr(R_HUBBLE)) * 2 * dsqr(pi);
   pref *= k_pivot * h_100;  // ???
   pref /= pow(k_pivot, n_spec);
...
void dump_param(cosmo* self, FILE *F)
{
   ...
   if (!F) F = stderr;
   fprintf(F, "#  O_m    O_de   w0_de  w1_de  h_100  O_b    O_nu   Neffnu sig_8  A_s  n_s "
         "nonlinear transfer growth de_param normmode N_a\n");
   fprintf(F, "# % .3f % .3f % .3f % .3f % .3f % .3f % .3f % .3f % .3f % .3e % .3f "
         "%s(%d) %s(%d) %s(%d) %s(%d) %d %d\n",
         self->Omega_m, self->Omega_de, self->w0_de, self->w1_de, self->h_100, self->Omega_b,
         self->Omega_nu_mass, self->Neff_nu_mass, self->sigma_8, self->As, self->n_spec,
...
double Esqr(cosmo* self, double a, int wOmegar, error **err)
{
   ...
   testErrorRetVA(!isfinite(EE), ce_infnan, "E^2 not finite for a=%g", *err, __LINE__, 0.0, a);
...
double P_L(cosmo* self, double a, double k, error **err)
{
   ...
   pl = P_L_nonorm(self, a, k, err);  /* set it to D^2(a) T^2(k) k^{n_s} */
   forwardError(*err,__LINE__,0.0);
   ...
   if (self->normmode == norm_s8) {
      ...
      forwardError(*err,__LINE__,0.0);
      // NEW (05/2016): sigma_8 moved here from P_L_nonorm
      return pl / s * dsqr(self->sigma_8);
      ...
      pl *= self->As;
      pl *= pref_pert_poiss(self->Omega_m, self->n_spec, self->h_100, k_pivot);
      return pl;
   }
}

/* ============================================================ *
 * New (05/2016): Returns D+^2(a) * k^n * T^2(k).			       *
 * No longer multiplied with sigma_8^2.                         *
...
/* Returns present-day dimensionless power spectrum k^3/(2pi^2) P(k, a=1) */
...
iter_start:
   ...
   if (fabs(r-i)<eps_a) {
      ...
      forwardError(*err, __LINE__, 0);
      testErrorRet(!isfinite(val), ce_infnan, "Power spectrum P_NL not finite", *err, __LINE__, -1);
      val = exp(val);
      return val;
   }
   else
   ...
   if (self->nonlinear == coyote10) {
      ...
      if (k < ksim_min / self->h_100) {
         ...
         /*
            ...
            self->w0_de, self->h_100, a, ksim_max / self->h_100, err);
            forwardError(*err, __LINE__, -1.0);
            ratio /= P_NL_fitting(tmp, a, ksim_max / self->h_100, err);
            forwardError(*err, __LINE__, -1.0);
            printf("ratio = %g\n", ratio);
            }
      ...
      if (k < fr_ksim_min / self->h_100) {

         /* Large scales -> linear power spectrum */
         val = P_L(self, a, k, err);   forwardError(*err, __LINE__, -1.0);

      } else if (k > fr_ksim_max / self->h_100) {
...
/* Return a(w) */
double a_of_w(cosmo *self, double ww, error **err)
{
   double a, a_low, a_high, da, wa, wmax;

   a_low  = self->a_min;
   a_high = 1.0;
   da     = 1.0e-8;

   testErrorRetVA(ww<0, ce_negative, "Comoving distance %g is negative",
      *err, __LINE__, -1.0, ww);
   wmax = w(self, a_low, 0, err);
   forwardError(*err, __LINE__, -1.0);
   testErrorRetVA(ww>wmax, ce_overflow, "Comoving distance %g larger than maximum distance %g (a_min=%g)",
      *err, __LINE__, -1.0, ww, wmax, a_low);
   ...
   do {
      ...
      intpar.self = self;
      intpar.i    = 0;
      wa = R_HUBBLE*sm2_qromberg(int_for_w, (void*)(&intpar), a, 1.0, 1.0e-8, err);
      */
      forwardError(*err, __LINE__, -1.0);
      if (ww < wa) {
         a_low = a;
...
/* Comoving angular diameter distance in Mpc/h. BS01 2.4, 2.30 */
...
/* Angular diameter distance (physical coordinates) in Mpc/h */
double D_a(cosmo *self, double a, error **err)
{
   double da, ww;
   ww   = w(self, a, 0, err);          forwardError(*err, __LINE__, 0);
   da = f_K(self, ww, err)*a;        forwardError(*err, __LINE__, 0);
   return da;
}

/* Angular diameter distance (physical coordinates) between two redshifts in Mpc/h see eq 19 in Hogg */
double D_a12(cosmo *self, double a1, double a2, error **err)
{
   double dm1,dm2, ww1, ww2,da, omega_k;
   ww1   = w(self, a1, 0, err);          forwardError(*err, __LINE__, 0);
   ww2   = w(self, a2, 0, err);          forwardError(*err, __LINE__, 0);
   dm1   = f_K(self, ww1, err);          forwardError(*err, __LINE__, 0);
   dm2   = f_K(self, ww2, err);          forwardError(*err, __LINE__, 0);

   /* Curvature without radiation (photons + massless neutrinos) density */
   omega_k = self->Omega_m + self->Omega_nu_mass + self->Omega_de - 1.0;

   testErrorRet(omega_k<0.0, ce_negative,
         "Angular diameter distance between 2 redshifts computed only for a universe with omega_k >= 0 ",
         *err, __LINE__, -1.0);

   da = a1 *( dm1 * sqrt(1.0 + omega_k*dm2*dm2/dsqr(R_HUBBLE)) - dm2 * sqrt(1.0 + omega_k*dsqr(dm1)/dsqr(R_HUBBLE)) );
