	       ...
	       poly_t poly,  int pm, error **err)
{
   ...
   testErrorRet(!isfinite(res), math_infnan, "R is not finite", *err, __LINE__,
		  0.0);
...
double Tplog_c(double z, const double *c, int n, error **err)
{
   ...
   testErrorRet(!isfinite(res), math_infnan, "Tplog is not finite", *err, __LINE__, 0.0);
...
double Tmlog(double z, const double *c, int n, error **err)
{
   ...
   testErrorRet(!isfinite(res), math_infnan, "Tmlog is not finite", *err, __LINE__, 0.0);
